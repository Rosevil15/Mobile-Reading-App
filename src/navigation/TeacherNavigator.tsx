...
type Screen = 'Dashboard' | 'StudentDetail' | 'AddMaterial' | 'Analytics' | 'Leaderboard' | 'Assignments' | 'QuizBuilder'

interface TeacherNavigatorProps { onLogout?: () => void }
...
export function TeacherNavigator({ onLogout }: TeacherNavigatorProps) {
  ...
  const titles: Record<Screen, string> = {
    Dashboard: 'Teacher Dashboard', StudentDetail: selectedStudent?.name ?? 'Student Detail',
    AddMaterial: 'Add Reading Material', Analytics: 'Progress Analytics',
    Leaderboard: 'Leaderboard', Assignments: 'Assignments', QuizBuilder: 'Quiz Builder',
  }
  ...
  if (screen === 'QuizBuilder') return <QuizBuilderScreen activeScreen={screen} title={titles[screen]} onNavigate={navigate} onLogout={onLogout ?? (() => {})} />
